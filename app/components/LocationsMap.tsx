...
interface LocationsMapProps {
  ...
  // Control externo del término de búsqueda (para mover la barra de búsqueda fuera del mapa)
  searchTerm?: string;
  onSearchChange?: (term: string) => void;
  // Ocultar la barra de búsqueda interna
  hideSearch?: boolean;
  // Modo del panel izquierdo: lista de ubicaciones o solo detalles de una ubicación
...
const LocationsMap: React.FC<LocationsMapProps> = ({ apiKey, locations, onSelectLocation, searchTerm, onSearchChange, hideSearch = false, panelMode = 'list' }) => {
  // Término de búsqueda local (fallback si no es controlado externamente)
  ...
  // Inicializar el mapa cuando la API de Google Maps está cargada
  useEffect(() => {
  ...
  // Actualizar marcadores cuando cambia el mapa o las ubicaciones
  useEffect(() => {
    ...
    // Crear nuevos marcadores para las ubicaciones filtradas
    filteredLocations.forEach(location => {
      ...
      marker.addListener('click', () => {
        ...
        // Guardar referencia a la ventana de info en el marcador
    ...
    // Si hay un location seleccionado, centrar el mapa en esa ubicación
    if (selectedLocation) {
  ...
  // Filtrar ubicaciones cuando cambia el término de búsqueda
  useEffect(() => {
    if (effectiveSearch === '') {
  ...
  // Manejar el clic en una ubicación de la lista
